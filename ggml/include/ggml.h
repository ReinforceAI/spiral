...
extern "C" {
    ...
    */
    ...
    // Spiral MPB rotation: signs → FWHT (or perm → signs → FWHT)
    // direction: 0=forward, 1=inverse
    // src[0] = input, src[1] = signs, src[2] = perm (NULL for no perm)
    GGML_API struct ggml_tensor * ggml_spiral_rotate(
            struct ggml_context * ctx,
            struct ggml_tensor  * a,
