...
// Spiral PQ2: 2-bit effective KV cache via product quantization
// Test 1: dummy identity — stores raw fp32, no compression
#define QK_SPIRAL_PQ2 128
#define NL_SPIRAL_PQ2     (QK_SPIRAL_PQ2 / 16)
#define NL_SPIRAL_PQ2_VEC (QK_SPIRAL_PQ2 / 4)

typedef struct {
    float d;                         // 4 bytes: L2 norm (unused in Test 1)
    uint8_t codes[32];               // 32 bytes: PQ codes (unused in Test 1)
} block_spiral_pq2;                  // 36 bytes total
static_assert(sizeof(block_spiral_pq2) == 36, "wrong spiral_pq2 block size");

// Spiral 3-bit: MPB-rotated 3-bit weight quantization (8-level Lloyd-Max for N(0,1))
// Block size 128: one block per rotation group (matches head_dim)
// Per block: norm(fp16) + 128x3-bit codes packed 8-per-3-bytes (48 bytes) = 50 bytes per 128 values
// = 3.125 bits/value -> 5.12x compression vs fp16
// Centroids are fixed constants: Lloyd-Max optimal for N(0,1)
// Dequant: value = centroid[code] * float(norm) / sqrt(in_features)
#define QK_SPIRAL_3BIT 128
typedef struct {
    ggml_half norm;                         //  2 bytes: per-row L2 norm
    uint8_t   qs[QK_SPIRAL_3BIT * 3 / 8];  // 48 bytes: packed 3-bit codes (8 codes per 3 bytes)
