...
// Spiral MPB rotation pipelines (block-128 WHT with signs + permutation)
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_spiral_rotate(ggml_metal_library_t lib, bool with_perm) {
    const char * base = with_perm ? "kernel_spiral_fwd_perm" : "kernel_spiral_fwd_pass";
    char name[256];
    snprintf(name, 256, "%s", base);
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_flash_attn_ext_vec(
    ...
    // Spiral fused FA-PQ kernel: K=SPIRAL_PQ2 with codebooks attached via src[5]
    // This kernel has its own dispatch logic and doesn't use function constants
    if (op->src[1]->type == GGML_TYPE_SPIRAL_PQ2 &&
        op->src[2]->type == GGML_TYPE_F16 &&
        op->src[5] != nullptr) {
        ggml_metal_pipeline_with_params res = ggml_metal_library_get_pipeline(lib, "kernel_flash_attn_ext_vec_spiral_pq2_fused");
        if (!res.pipeline) {
            res = ggml_metal_library_compile_pipeline(lib, "kernel_flash_attn_ext_vec_spiral_pq2_fused",
