...
// SpiralFlash two-pass: K=SPIRAL_PQ2, V=F16 fused attention
// Pass 1: PQ decode + R_inv rotation + mean + RoPE + scoring + partial V accumulation
// Pass 2: reuses ggml_metal_kargs_turbo_flash_p2 (same merge logic, no WHT)
typedef struct {
    int32_t  ne01;      // n_queries
    int32_t  ne02;      // n_q_heads
    int32_t  ne03;      // n_batch
    uint64_t nb01;      // Q stride (query)
    uint64_t nb02;      // Q stride (head)
    uint64_t nb03;      // Q stride (batch)
    int32_t  ne11;      // n_kv (total KV positions)
    int32_t  ne_12_2;   // n_kv_heads
    int32_t  ne_12_3;   // n_kv_batch
    uint64_t nb11;      // K stride (position) — after permute
    uint64_t nb12;      // K stride (head) — after permute
    uint64_t nb13;      // K stride (batch)
    uint64_t nb21;      // V stride (position) — after permute
    uint64_t nb22;      // V stride (head) — after permute
    uint64_t nb23;      // V stride (batch)
    int32_t  ne31;      // mask ne[1]
    int32_t  ne32;      // mask ne[2]
    int32_t  ne33;      // mask ne[3]
    uint64_t nb31;      // mask stride 1
    uint64_t nb32;      // mask stride 2
    uint64_t nb33;      // mask stride 3
