...
int ggml_metal_op_spiral_rotate(ggml_metal_op_t ctx, int idx);
...
static int ggml_metal_op_encode_impl(ggml_metal_op_t ctx, int idx) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_SPIRAL_ROTATE:
            {
                n_fuse = ggml_metal_op_spiral_rotate(ctx, idx);
            } break;
...
int ggml_metal_op_set_rows(ggml_metal_op_t ctx, int idx) {
    ...
    // Check for Spiral PQ codebook variant:
    // src[3] = codebook, src[4] = mean, src[5] = R_kv
    const bool has_spiral_cb = (op->src[3] != nullptr &&
                                op->type == GGML_TYPE_SPIRAL_PQ2);

    ggml_metal_pipeline_with_params pipeline;
    if (has_spiral_cb) {
        // Use the codebook variant kernel
        char base[256];
        char name[256];
        snprintf(base, 256, "kernel_set_rows_spiral_pq2_cb_%s", ggml_type_name(op->src[1]->type));
        snprintf(name, 256, "%s", base);
        pipeline = ggml_metal_library_get_pipeline(lib, name);
        if (!pipeline.pipeline) {
            pipeline = ggml_metal_library_compile_pipeline(lib, base, name, nullptr);
        }
    } else {
        pipeline = ggml_metal_library_get_pipeline_set_rows(lib, op->src[1]->type, op->type);
    }
    ...
    if (has_spiral_cb) {
        // Pass codebook, mean, and rotation buffers
        ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[3]), 4); // codebooks
        ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[4]), 5); // means
        ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[5]), 6); // R_kv
    }
...
int ggml_metal_op_spiral_rotate(ggml_metal_op_t ctx, int idx) {
    ...
    const bool has_perm = (op->src[2] != nullptr);
    const int64_t n_elements = ggml_nelements(op->src[0]);
    const int64_t D = op->src[0]->ne[0];  // dimension (signs array length)
    const int64_t n_tg = n_elements / 128;
    ...
    int ida = 0;
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[0]), ida++); // input
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op),         ida++); // output
    ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[1]), ida++); // signs
    if (has_perm) {
        ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[2]), ida++); // perm
    }

    int64_t D_val = D;
    ggml_metal_encoder_set_bytes(enc, &D_val, sizeof(D_val), ida++);

    ggml_metal_encoder_dispatch_threadgroups(enc, (int)n_tg, 1, 1, 128, 1, 1);
    return 1;
}
...
int ggml_metal_op_cpy(ggml_metal_op_t ctx, int idx) {
    ...
    // Check for Spiral PQ codebook dequant variant:
    // src[2] = codebooks, src[3] = R_kv_inv, src[4] = means
    const bool has_spiral_cb = (op->src[2] != nullptr &&
                                op->src[0]->type == GGML_TYPE_SPIRAL_PQ2 &&
                                op->type == GGML_TYPE_F32);

    if (has_spiral_cb) {
        // Use the PQ codebook dequant kernel
        char name[256];
        snprintf(name, 256, "kernel_cpy_spiral_pq2_cb_f32");
        auto pipeline = ggml_metal_library_get_pipeline(lib, name);
        if (!pipeline.pipeline) {
            pipeline = ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
        }

        // PQ decode is single-threaded per (head, position) — the kernel does 128×128 matmul
        // Dispatch: one threadgroup per (head, kv_position)
        // ne01 = n_kv_heads, ne02 = n_kv_positions
        ggml_metal_kargs_cpy args = {
            /*.nk0  =*/ ne00/16,  // nk0 = 8 (128/16), but PQ kernel ignores this
            /*.ne00 =*/ ne00,
            /*.ne01 =*/ ne01,
            /*.ne02 =*/ ne02,
            /*.ne03 =*/ ne03,
        ...
        ggml_metal_encoder_set_pipeline(enc, pipeline);
        ggml_metal_encoder_set_bytes   (enc, &args, sizeof(args), 0);
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[0]), 1);  // SPIRAL_PQ2 source
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op),         2);  // F32 destination
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[2]), 3);  // codebooks
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[3]), 4);  // R_kv_inv
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[4]), 5);  // means

        // Dispatch: 128 threads per threadgroup (one per output dimension)
        // threadgroups = (ne01, ne02, ne03) = (n_kv_heads, n_kv_positions, 1)
        ggml_metal_encoder_dispatch_threadgroups(enc, ne01, ne02, ne03, 128, 1, 1);
...
int ggml_metal_op_mul_mat(ggml_metal_op_t ctx, int idx) {
    ...
    if (op->src[1]->type == GGML_TYPE_F32 && (ne00%128 == 0) &&
        ...
        switch (ne11) {
            case 1:
                r1ptg = 2; break;  // pad to 2 — kernel handles bounds
            case 2:
                r1ptg = 2; break;
            case 3:
            case 6:
        ...
        auto pipeline = ggml_metal_library_get_pipeline_mul_mv_ext(lib, op->src[0]->type, op->src[1]->type, nsg, nxpsg, r1ptg);        ggml_metal_kargs_mul_mv_ext args = {
            /*.ne00  =*/ ne00,
            /*.ne01  =*/ ne01,
            /*.ne02  =*/ ne02,
            /*.nb00  =*/ nb00,
        ...
        if (is_tq_weight && ne00 % 32 == 0) {
            ...
            // TQ rotation: single-pass block-32 WHT with hardcoded signs
        ...
        if (is_tq_weight && ne00 % 32 == 0) {
            ...
            // TQ: inverse-rotate src1 to restore original values (shared activation)
...
// Only for decode (ne01=1), head_dim=128, with codebooks attached
static bool ggml_metal_op_flash_attn_ext_use_spiral_flash(const ggml_tensor * op) {
    assert(op->op == GGML_OP_FLASH_ATTN_EXT);

    // Two-pass SpiralFlash is disabled by default — single-pass fused kernel is faster
    // at short/medium context. Enable with SPIRAL_FLASH=1 for long-context (n_kv > 1024)
    // where block-parallel Pass 1 provides better GPU occupancy.
    const char * env = getenv("SPIRAL_FLASH");
    if (env && env[0] == '1') {
        const int64_t ne01 = op->src[0]->ne[1];
        const int64_t ne00 = op->src[0]->ne[0];
        if (ne01 != 1) return false;
        if (op->src[1]->type != GGML_TYPE_SPIRAL_PQ2) return false;
        if (op->src[5] == nullptr) return false;
...
size_t ggml_metal_op_flash_attn_ext_extra_tmp(const ggml_tensor * op) {
  ...
  //GGML_TENSOR_LOCALS(uint64_t, nb3, op->src[3], nb);
    ...
    // SpiralFlash two-pass temp — same layout as TurboFlash
    if (ggml_metal_op_flash_attn_ext_use_spiral_flash(op)) {
        const int64_t n_bh = ne01 * ne02 * ne03;
        const int64_t ne11 = op->src[1]->ne[1];  // n_kv
        const int64_t n_blocks = (ne11 + 255) / 256;  // must match SPIRAL_FLASH_BLOCK_SIZE
        const int64_t dv = ne20;
...
int ggml_metal_op_flash_attn_ext(ggml_metal_op_t ctx, int idx) {
    ...
    // =========================================================================
    // SpiralFlash two-pass: K=PQ2 decode (ne01=1) with block-parallel attention
    // =========================================================================
    if (ggml_metal_op_flash_attn_ext_use_spiral_flash(op)) {
        const int32_t ne01 = op->src[0]->ne[1]; // n_queries (=1 for decode)
        const int32_t ne02 = op->src[0]->ne[2]; // n_q_heads
        const int32_t ne03 = op->src[0]->ne[3]; // n_batch
        const int32_t ne11 = op->src[1]->ne[1]; // n_kv (after permute = n_kv)
        const int32_t ne12 = op->src[1]->ne[2]; // n_kv_heads (after permute)
        const int32_t ne13 = op->src[1]->ne[3];
        const int32_t ne20 = op->src[2]->ne[0]; // V head_dim
        ...
        const int32_t n_bh = ne01 * ne02 * ne03;

        // Compute temp buffer offsets — same layout as TurboFlash
        // VEC tmp comes first, then our partials
        const int64_t nwg = 32;
        ...
        ggml_metal_buffer_id bid_spiral_base = bid_tmp;
        bid_spiral_base.offs += vec_tmp_size;

        // Also skip TurboFlash tmp if it was allocated
        if (ggml_metal_op_flash_attn_ext_use_turbo_flash(op)) {
            const int64_t tf_n_bh = ne01 * ne02 * ne03;
            const int64_t tf_n_blocks = (ne11 + 63) / 64;
            const int64_t tf_dv = ne20;
            bid_spiral_base.offs += sizeof(float) * tf_n_bh * tf_n_blocks * (tf_dv + 2);
        }

        const size_t partial_out_bytes = sizeof(float) * n_bh * n_blocks * ne20;

        ggml_metal_buffer_id bid_partial_out = bid_spiral_base;
        ggml_metal_buffer_id bid_partial_ms  = bid_spiral_base;
        bid_partial_ms.offs += partial_out_bytes;

        // ---- Pass 1: Block scoring with PQ K decode ----
        {
            ggml_metal_kargs_spiral_flash_p1 args_p1 = {
                /*.ne01      =*/ ne01,
                /*.ne02      =*/ ne02,
                /*.ne03      =*/ ne03,
                /*.nb01      =*/ op->src[0]->nb[1],
                ...
                /*.nb22      =*/ op->src[2]->nb[2],
                /*.nb23      =*/ op->src[2]->nb[3],
                /*.ne31      =*/ op->src[3] ? (int32_t)op->src[3]->ne[1] : (int32_t)0,
                /*.ne32      =*/ op->src[3] ? (int32_t)op->src[3]->ne[2] : (int32_t)1,
                /*.ne33      =*/ op->src[3] ? (int32_t)op->src[3]->ne[3] : (int32_t)1,
                /*.nb31      =*/ op->src[3] ? op->src[3]->nb[1] : (uint64_t)0,
            ...
            auto pipeline = ggml_metal_library_get_pipeline(lib, "kernel_spiral_pq2_flash_p1");
            if (!pipeline.pipeline) {
                pipeline = ggml_metal_library_compile_pipeline(lib, "kernel_spiral_pq2_flash_p1",
                                                                "kernel_spiral_pq2_flash_p1", nullptr);
            }
            if (!pipeline.pipeline) {
                goto spiral_flash_end;
            }

            const size_t smem_p1 = (128 + 128 + 4) * sizeof(float); // s_rot[128] + s_k[128] + s_reduce[4]
            ...
            ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[1]), 2);  // K (PQ2)
            ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[2]), 3);  // V (F16)
            ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[3] ? op->src[3] : op->src[0]), 4);  // mask
            ...
            ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[5]), 7);  // codebooks
            ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[6]), 8);  // R_kv_inv
            ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[7]), 9);  // means
            ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[8]), 10); // positions

            ggml_metal_encoder_set_threadgroup_memory_size(enc, smem_p1, 0);

            // Grid: (n_bh, n_blocks, 1), TG: (128, 1, 1) — 4 SIMD groups
            ggml_metal_encoder_dispatch_threadgroups(enc, n_bh, n_blocks, 1, 128, 1, 1);
        }

        // Barrier between Pass 1 and Pass 2
        ggml_metal_op_concurrency_reset(ctx);

        // ---- Pass 2: Merge partials + write output (no WHT) ----
        {
            ...
            auto pipeline = ggml_metal_library_get_pipeline(lib, "kernel_spiral_pq2_flash_p2");
            if (!pipeline.pipeline) {
                pipeline = ggml_metal_library_compile_pipeline(lib, "kernel_spiral_pq2_flash_p2",
                                                                "kernel_spiral_pq2_flash_p2", nullptr);
            }
            if (!pipeline.pipeline) {
                goto spiral_flash_end;
            }

            const size_t smem_p2 = sizeof(float) * (128 + 2); // shared_out[128] + max + sum
            ...
            // Grid: (n_bh, 1, 1), TG: (128, 1, 1)
            ggml_metal_encoder_dispatch_threadgroups(enc, n_bh, 1, 1, 128, 1, 1);
        }
    ...
    // =========================================================================
    // Fused Spiral PQ2 FA (single-pass): handles both decode and prefill
    // Two-pass SpiralFlash available via SPIRAL_FLASH=1 for long-context decode
    // =========================================================================
    if (op->src[1]->type == GGML_TYPE_SPIRAL_PQ2 && op->src[5] != nullptr) {
        // K is SPIRAL_PQ2 with codebooks — use fused kernel
        // src[0]=Q, src[1]=K(PQ2), src[2]=V(F16), src[3]=mask, src[4]=sinks
        // src[5]=codebooks, src[6]=R_kv_inv, src[7]=means, src[8]=positions

        const int32_t ne01 = op->src[0]->ne[1]; // n_queries
        const int32_t ne02 = op->src[0]->ne[2]; // n_heads
        const int32_t ne03 = op->src[0]->ne[3]; // n_batch
        const int32_t ne11 = op->src[1]->ne[1]; // after permute: what is this?
        ...
        const int32_t ne1 = op->ne[1];
        const int32_t ne2 = op->ne[2];
        const int32_t ne3 = op->ne[3];

        const int32_t n_head      = ne02;
        const int32_t n_head_log2 = 1u << (uint32_t) floorf(log2f((float) n_head));
        const float m0 = powf(2.0f, -(max_bias       ) / n_head_log2);
        const float m1 = powf(2.0f, -(max_bias / 2.0f) / n_head_log2);
        ...
        ggml_metal_kargs_flash_attn_ext_vec args = {
            ...
            /*.ne_12_2       =*/ ne12,
            /*.ne_12_3       =*/ ne13,
            /*.ns10          =*/ int32_t(op->src[1]->nb[1] / op->src[1]->nb[0]),
            /*.nb11          =*/ op->src[1]->nb[1],
            /*.nb12          =*/ op->src[1]->nb[2],
            /*.nb13          =*/ op->src[1]->nb[3],
            /*.ns20          =*/ int32_t(op->src[2]->nb[1] / op->src[2]->nb[0]),
            /*.nb21          =*/ op->src[2]->nb[1],
            /*.nb22          =*/ op->src[2]->nb[2],
            /*.nb23          =*/ op->src[2]->nb[3],
            /*.ne31          =*/ op->src[3] ? (int32_t)op->src[3]->ne[1] : (int32_t)0,
        ...
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[1]), 2);   // K (PQ2)
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[2]), 3);   // V (F16)
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[3] ? op->src[3] : op->src[0]), 4);   // mask (or Q as dummy)
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[4] ? op->src[4] : op->src[0]), 5);   // sinks (or Q as dummy)
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[5]), 6);   // codebooks
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[6]), 7);   // R_kv_inv
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[7]), 8);   // means
    ...
    // Allow asymmetric K/V quantization for supported mixed pairs
    {
        ...
        if (type_k != type_v) {
            ...
            const bool k_is_f16_v_is_spiral = (type_k == GGML_TYPE_F16 && type_v == GGML_TYPE_SPIRAL_PQ2);
            const bool k_is_spiral_v_is_f16 = (type_k == GGML_TYPE_SPIRAL_PQ2 && type_v == GGML_TYPE_F16);
            const bool supported = (k_is_turbo && v_is_turbo) ||
                                   (k_is_q8 && v_is_turbo) ||
                                   (k_is_turbo && v_is_q8) ||
                                   k_is_f16_v_is_spiral ||
                                   k_is_spiral_v_is_f16;
            GGML_ASSERT(supported && "asymmetric K/V types only supported for turbo, q8_0, and spiral mixed pairs");
        }
    }
