...
// =====================================================================
// Spiral PQ2 dequantize for flash attention
// Test 2: 2-bit scalar Lloyd-Max centroid lookup
// =====================================================================
constant float spiral_dequant_2bit[4] = { -0.133462f, -0.039994f, 0.039994f, 0.133462f };

template <typename type4x4>
void dequantize_spiral_pq2(device const block_spiral_pq2 * xb, short il, thread type4x4 & reg) {
    const float norm = xb->d;
    const int qs_off = il * 4;
    float4x4 reg_f;
    for (int g = 0; g < 4; g++) {
        const uint8_t qb = xb->codes[qs_off + g];
        reg_f[g] = float4(
            spiral_dequant_2bit[(qb      ) & 0x03] * norm,
            spiral_dequant_2bit[(qb >> 2) & 0x03] * norm,
            spiral_dequant_2bit[(qb >> 4) & 0x03] * norm,
            spiral_dequant_2bit[(qb >> 6)       ] * norm
        );
    }
    reg = (type4x4) reg_f;
}
...
template <typename type4>
void dequantize_spiral_pq2_t4(device const block_spiral_pq2 * xb, short il, thread type4 & reg) {
    const float norm = xb->d;
    const uint8_t qb = xb->codes[il];
    reg = type4(float4(
        spiral_dequant_2bit[(qb      ) & 0x03] * norm,
        spiral_dequant_2bit[(qb >> 2) & 0x03] * norm,
        spiral_dequant_2bit[(qb >> 4) & 0x03] * norm,
...
// Spiral Lloyd-Max optimal centroids for 3-bit (8 levels) on N(0,1) distribution
constant float spiral_centroids_3bit[8] = {
...
// Spiral 3-bit: unpack 8 indices from 3 bytes (same bit layout as TQ3)
inline void spiral_3bit_unpack_group(device const uint8_t * qp, thread uint8_t * idx) {
    idx[0] =  qp[0]       & 7;
    idx[1] = (qp[0] >> 3) & 7;
    idx[2] = ((qp[0] >> 6) | (qp[1] << 2)) & 7;
    idx[3] = (qp[1] >> 1) & 7;
...
// =====================================================================
// SPIRAL 3-BIT dequant functions
// Block size 128, single per-block norm, Lloyd-Max centroids
// =====================================================================

// SPIRAL_3BIT: 4x4 dequant for pre-rotated activations (no inverse rotation)
// il ranges 0..7 (128/16 = 8 steps), each call decodes 16 elements
template <typename type4x4>
void dequantize_spiral_3bit_rotated(device const block_spiral_3bit * xb, short il, thread type4x4 & reg) {
    const float norm = float(xb->norm);
    const int byte_off = il * 6;  // 2 groups × 3 bytes = 6 bytes per il step

    float4x4 reg_f;

    // Group 0: 8 codes from bytes [byte_off .. byte_off+2]
    {
        uint8_t idx[8];
        spiral_3bit_unpack_group(xb->qs + byte_off, idx);
        reg_f[0] = float4(
            spiral_centroids_3bit[idx[0]] * norm,
            spiral_centroids_3bit[idx[1]] * norm,
            spiral_centroids_3bit[idx[2]] * norm,
            spiral_centroids_3bit[idx[3]] * norm
        );
        reg_f[1] = float4(
            spiral_centroids_3bit[idx[4]] * norm,
            spiral_centroids_3bit[idx[5]] * norm,
            spiral_centroids_3bit[idx[6]] * norm,
            spiral_centroids_3bit[idx[7]] * norm
        );
    }

    // Group 1: 8 codes from bytes [byte_off+3 .. byte_off+5]
    {
        uint8_t idx[8];
        spiral_3bit_unpack_group(xb->qs + byte_off + 3, idx);
        reg_f[2] = float4(
            spiral_centroids_3bit[idx[0]] * norm,
            spiral_centroids_3bit[idx[1]] * norm,
            spiral_centroids_3bit[idx[2]] * norm,
...
// SPIRAL_3BIT: t4 dequant for pre-rotated activations
// il ranges 0..31 (128/4 = 32 steps), each call decodes 4 elements
template <typename type4>
void dequantize_spiral_3bit_rotated_t4(device const block_spiral_3bit * xb, short il, thread type4 & reg) {
    const float norm = float(xb->norm);
    const short group = il / 2;       // which group of 8 (0..15)
    const short g_off = (il % 2) * 4; // first 4 or second 4

    uint8_t idx[8];
    spiral_3bit_unpack_group(xb->qs + group * 3, idx);

    reg = (type4) float4(
        spiral_centroids_3bit[idx[g_off + 0]] * norm,
        spiral_centroids_3bit[idx[g_off + 1]] * norm,
        spiral_centroids_3bit[idx[g_off + 2]] * norm,
        spiral_centroids_3bit[idx[g_off + 3]] * norm
    );
}

// Non-rotated aliases (rotation handled at graph level for now)
template <typename type4x4>
void dequantize_spiral_3bit(device const block_spiral_3bit * xb, short il, thread type4x4 & reg) {
    dequantize_spiral_3bit_rotated(xb, il, reg);
}

template <typename type4>
void dequantize_spiral_3bit_t4(device const block_spiral_3bit * xb, short il, thread type4 & reg) {
    dequantize_spiral_3bit_rotated_t4(xb, il, reg);
}
...
// =====================================================================
// Spiral MPB rotation kernels for block-128 WHT
//
// Forward:  fwd_pass(x, tmp, signs1, D)  →  fwd_perm(tmp, x, signs2, perm, D)
// Inverse:  inv_pass(x, tmp, signs2, D)  →  inv_perm_pass(tmp, x, signs1, inv_perm, D)
//
// Dispatch per kernel: grid=(n_elements/128, 1, 1), threads=(128, 1, 1)
// =====================================================================

// Forward: x_out = FWHT(x_in * signs) — no permutation
kernel void kernel_spiral_fwd_pass(
        device const float   * x_in      [[buffer(0)]],
        device       float   * x_out     [[buffer(1)]],
        device const float   * signs     [[buffer(2)]],
        constant int64_t     & D         [[buffer(3)]],
        uint   tg_id [[threadgroup_position_in_grid]],
        uint   lane  [[thread_position_in_threadgroup]]) {

    constexpr int BLOCK = 128;
    constexpr int LOG2_BLOCK = 7;
    const float inv_sqrt2 = 0.70710678118654752440f;
    threadgroup float s[BLOCK];

    uint global_idx = tg_id * BLOCK + lane;
    uint dim_idx = global_idx % (uint)D;

    s[lane] = x_in[global_idx] * signs[dim_idx];
    threadgroup_barrier(mem_flags::mem_threadgroup);

    for (int stage = 0; stage < LOG2_BLOCK; stage++) {
        uint h = 1u << stage;
        uint partner = lane ^ h;
        float mine = s[lane];
        float other = s[partner];
        threadgroup_barrier(mem_flags::mem_threadgroup);
        s[lane] = ((lane & h) == 0) ? (mine + other) * inv_sqrt2
                                     : (other - mine) * inv_sqrt2;
        threadgroup_barrier(mem_flags::mem_threadgroup);
    }

    x_out[global_idx] = s[lane];
}

// Forward with permutation: x_out = FWHT(x_in[perm] * signs)
kernel void kernel_spiral_fwd_perm(
        device const float   * x_in      [[buffer(0)]],
        device       float   * x_out     [[buffer(1)]],
        device const float   * signs     [[buffer(2)]],
        device const int32_t * perm      [[buffer(3)]],
        constant int64_t     & D         [[buffer(4)]],
        uint   tg_id [[threadgroup_position_in_grid]],
        uint   lane  [[thread_position_in_threadgroup]]) {
    ...
    uint global_idx = tg_id * BLOCK + lane;
    uint dim_idx = global_idx % (uint)D;
    uint batch_offset = (global_idx / (uint)D) * (uint)D;

    uint src_idx = batch_offset + (uint)perm[dim_idx];
    s[lane] = x_in[src_idx] * signs[dim_idx];
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
// Inverse: x_out = FWHT(x_in) * signs — no permutation
kernel void kernel_spiral_inv_pass(
        device const float   * x_in      [[buffer(0)]],
        device       float   * x_out     [[buffer(1)]],
        device const float   * signs     [[buffer(2)]],
        constant int64_t     & D         [[buffer(3)]],
    ...
    x_out[global_idx] = s[lane] * signs[dim_idx];
}

// Inverse with permutation: x_out = FWHT(x_in[inv_perm]) * signs
kernel void kernel_spiral_inv_perm_pass(
        device const float   * x_in      [[buffer(0)]],
        device       float   * x_out     [[buffer(1)]],
        device const float   * signs     [[buffer(2)]],
        device const int32_t * inv_perm  [[buffer(3)]],
        constant int64_t     & D         [[buffer(4)]],
        uint   tg_id [[threadgroup_position_in_grid]],
        uint   lane  [[thread_position_in_threadgroup]]) {
    ...
    uint src_idx = batch_offset + (uint)inv_perm[dim_idx];
    s[lane] = x_in[src_idx];
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
// ============================================================================
// SPIRAL_3BIT dedicated mat-vec kernel for decode (ne11=1)
// ============================================================================

template<short nr0, typename args_t>
void kernel_mul_mv_spiral_3bit_f32_impl(
        args_t args,
        device const char * src0,
        device const char * src1,
        device       char * dst,
    ...
    device const block_spiral_3bit * x = (device const block_spiral_3bit *) (src0 + offset0);
    device const float             * y = (device const float             *) (src1 + offset1);
    ...
    const short g0 = it * 2;
    const short g1 = it * 2 + 1;
    const short boff0 = g0 * 3;
    const short boff1 = g1 * 3;

    device const float * yp = y + ix * 128 + it * 16;

    const int row_stride = (int)(args.nb01 / sizeof(block_spiral_3bit));
    ...
    for (int ib = ix; ib < nb; ib += 4) {
        ...
        device const block_spiral_3bit * xr = x + ib;

        for (short row = 0; row < nr0; row++) {
            const float norm = float(xr->norm);

            const uint8_t a0 = xr->qs[boff0];
            const uint8_t a1 = xr->qs[boff0 + 1];
            const uint8_t a2 = xr->qs[boff0 + 2];

            float acc = 0.0f;
            acc += c[ a0       & 7] * yl[0];
            acc += c[(a0 >> 3) & 7] * yl[1];
            acc += c[((a0 >> 6) | (a1 << 2)) & 7] * yl[2];
            acc += c[(a1 >> 1) & 7] * yl[3];
            acc += c[(a1 >> 4) & 7] * yl[4];
            acc += c[((a1 >> 7) | (a2 << 1)) & 7] * yl[5];
            acc += c[(a2 >> 2) & 7] * yl[6];
            acc += c[(a2 >> 5) & 7] * yl[7];

            const uint8_t b0 = xr->qs[boff1];
            const uint8_t b1 = xr->qs[boff1 + 1];
            const uint8_t b2 = xr->qs[boff1 + 2];

            acc += c[ b0       & 7] * yl[8];
            acc += c[(b0 >> 3) & 7] * yl[9];
            acc += c[((b0 >> 6) | (b1 << 2)) & 7] * yl[10];
            acc += c[(b1 >> 1) & 7] * yl[11];
            acc += c[(b1 >> 4) & 7] * yl[12];
            acc += c[((b1 >> 7) | (b2 << 1)) & 7] * yl[13];
            acc += c[(b2 >> 2) & 7] * yl[14];
            acc += c[(b2 >> 5) & 7] * yl[15];
...
[[host_name("kernel_mul_mv_spiral_3bit_f32")]]
kernel void kernel_mul_mv_spiral_3bit_f32(
        ...
        ushort tiisg [[thread_index_in_simdgroup]],
        ushort sgitg [[simdgroup_index_in_threadgroup]]) {
    kernel_mul_mv_spiral_3bit_f32_impl<N_R0_SPIRAL_3BIT, constant ggml_metal_kargs_mul_mv &>(
...
template [[host_name("kernel_mul_mv_ext_spiral_3bit_f32_r1_2")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<2, block_spiral_3bit, 128, dequantize_spiral_3bit_rotated_t4>;
template [[host_name("kernel_mul_mv_ext_spiral_3bit_f32_r1_3")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<3, block_spiral_3bit, 128, dequantize_spiral_3bit_rotated_t4>;
template [[host_name("kernel_mul_mv_ext_spiral_3bit_f32_r1_4")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<4, block_spiral_3bit, 128, dequantize_spiral_3bit_rotated_t4>;
template [[host_name("kernel_mul_mv_ext_spiral_3bit_f32_r1_5")]] kernel mul_mv_ext_q4_f32_t kernel_mul_mv_ext_q4_f32_disp<5, block_spiral_3bit, 128, dequantize_spiral_3bit_rotated_t4>;
...
// Spiral PQ2 flash attention
template [[host_name("kernel_flash_attn_ext_kspiral_pq2_vspiral_pq2_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, block_spiral_pq2, NL_SPIRAL_PQ2, dequantize_spiral_pq2, block_spiral_pq2, NL_SPIRAL_PQ2, dequantize_spiral_pq2, 128, 128>;

// Spiral pre-RoPE path: K is dequanted to F16 at graph level, V stays SPIRAL_PQ2 in FA
template [[host_name("kernel_flash_attn_ext_kf16_vspiral_pq2_dk128_dv128")]] kernel flash_attn_ext_t kernel_flash_attn_ext<FA_TYPES, half4x4, 1, dequantize_f16, block_spiral_pq2, NL_SPIRAL_PQ2, dequantize_spiral_pq2, 128, 128>;
...
template [[host_name("kernel_flash_attn_ext_vec_kspiral_pq2_vspiral_pq2_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, block_spiral_pq2, NL_SPIRAL_PQ2_VEC, dequantize_spiral_pq2_t4, block_spiral_pq2, NL_SPIRAL_PQ2_VEC, dequantize_spiral_pq2_t4, 128, 128, 1>;

// Spiral pre-RoPE path: K=F16 (graph-level dequant+rotate+RoPE), V=SPIRAL_PQ2 (FA-level dequant)
template [[host_name("kernel_flash_attn_ext_vec_kf16_vspiral_pq2_dk128_dv128")]] kernel flash_attn_ext_vec_t kernel_flash_attn_ext_vec<FA_TYPES, half4, 1, dequantize_f16_t4, block_spiral_pq2, NL_SPIRAL_PQ2_VEC, dequantize_spiral_pq2_t4, 128, 128, 1>;
...
// =============================================================================
// Fused Flash Attention for Spiral PQ2 K (pre-RoPE) + F16 V
// =============================================================================
// Replaces graph-level: ggml_cast(PQ2→F32) + ggml_rope_ext + ggml_cast(F32→F16)
// K stays as SPIRAL_PQ2 in KV cache. V is pre-cast to F16 at graph level.
//
// Extra buffers beyond standard FA:
//   buffer(6): codebooks [n_kv_heads, 32, 256, 4] F32
//   buffer(7): R_kv_inv  [128, 128] F32
//   buffer(8): means     [n_kv_heads, 128] F32
//   buffer(9): positions [n_kv] I32
// freq_base and n_rot passed via op_params (buffer 0)
// =============================================================================

kernel void kernel_flash_attn_ext_vec_spiral_pq2_fused(
    ...
    constexpr short NBLK = 32;
    constexpr short NCW  = 256;
    constexpr short PBS  = 4;
    ...
    // Shared memory: s_rot[128] + sq[128] + so[128] = 384 floats
    ...
    const float mean0 = head_mean[4*tx + 0];
    const float mean1 = head_mean[4*tx + 1];
    const float mean2 = head_mean[4*tx + 2];
    const float mean3 = head_mean[4*tx + 3];
    ...
    // Each thread handles 4 contiguous dims: d_base, d_base+1, d_base+2, d_base+3
    const int d_base_rope = 4 * tx;
    const bool first_half_rope = (d_base_rope < D / 2);
    const int freq_dim = first_half_rope ? d_base_rope : (d_base_rope - D/2);
    const float rope_freq0 = 1.0f / pow(rope_freq_base, (float)(2 * freq_dim) / (float)D);
    const float rope_freq1 = 1.0f / pow(rope_freq_base, (float)(2 * (freq_dim+1)) / (float)D);
    const float rope_freq2 = 1.0f / pow(rope_freq_base, (float)(2 * (freq_dim+2)) / (float)D);
    const float rope_freq3 = 1.0f / pow(rope_freq_base, (float)(2 * (freq_dim+3)) / (float)D);
    const short rope_partner = first_half_rope ? (tx + 16) : (tx - 16);
    ...
    for (int ic = 0; ic < args.ne11; ic++) {
        ...
        // Step 1: PQ codebook lookup → s_rot
        {
            ...
            const uint8_t code = blk->codes[tx];
            device const float * cb_vec = head_cb + tx * NCW * PBS + code * PBS;
            s_rot[tx * PBS + 0] = cb_vec[0] * norm;
            s_rot[tx * PBS + 1] = cb_vec[1] * norm;
            s_rot[tx * PBS + 2] = cb_vec[2] * norm;
            s_rot[tx * PBS + 3] = cb_vec[3] * norm;
        }

        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Step 2: Direct read from PQ decoded vector (natural space — no R_inv needed)
        const int d_base = 4 * tx;
        float k0 = s_rot[d_base + 0];
        float k1 = s_rot[d_base + 1];
        float k2 = s_rot[d_base + 2];
        float k3 = s_rot[d_base + 3];
        ...
        // Step 4: RoPE (NEOX: dim i pairs with dim i+D/2)
        // Frequencies precomputed before loop — only cos/sin computed per position
        {
            const int pos = positions[ic];

            const float pk0 = simd_shuffle(k0, rope_partner);
            const float pk1 = simd_shuffle(k1, rope_partner);
            const float pk2 = simd_shuffle(k2, rope_partner);
            const float pk3 = simd_shuffle(k3, rope_partner);

            const float t0 = (float)pos * rope_freq0;
            const float t1 = (float)pos * rope_freq1;
            const float t2 = (float)pos * rope_freq2;
            const float t3 = (float)pos * rope_freq3;

            if (first_half_rope) {
                const float a0=k0, a1=k1, a2=k2, a3=k3;
                k0 = a0*cos(t0) - pk0*sin(t0);
                k1 = a1*cos(t1) - pk1*sin(t1);
                k2 = a2*cos(t2) - pk2*sin(t2);
                k3 = a3*cos(t3) - pk3*sin(t3);
            } else {
                const float a0=k0, a1=k1, a2=k2, a3=k3;
                k0 = pk0*sin(t0) + a0*cos(t0);
                k1 = pk1*sin(t1) + a1*cos(t1);
                k2 = pk2*sin(t2) + a2*cos(t2);
                k3 = pk3*sin(t3) + a3*cos(t3);
            }
        }

        // Step 5: Q · K dot product
        float qk = sq[4*tx+0] * k0 + sq[4*tx+1] * k1 +
                    sq[4*tx+2] * k2 + sq[4*tx+3] * k3;
        qk = simd_sum(qk);
        ...
        // Step 6: Online softmax + V accumulation
        {
            ...
            so[4*tx + 0] += vs * (float)v_row[4*tx + 0];
            so[4*tx + 1] += vs * (float)v_row[4*tx + 1];
            so[4*tx + 2] += vs * (float)v_row[4*tx + 2];
            so[4*tx + 3] += vs * (float)v_row[4*tx + 3];
        }
    ...
    if (S > 0.0f) {
        ...
        // Output layout: [D, n_heads, n_queries, batch]
        // ggml stores ne[0] fastest, so offset = batch*ne2*ne1*D + query*ne1*D + head*D + d
        // iq1 = query (tgpig[0]), iq2 = head (tgpig[1]), iq3 = batch (tgpig[2])
        device float * out = (device float *)(dst) +
            (uint64_t)iq3 * args.ne2 * args.ne1 * D +
            (uint64_t)iq1 * args.ne1 * D +
            (uint64_t)iq2 * D;

        out[4*tx + 0] = so[4*tx + 0] * inv_S;
        out[4*tx + 1] = so[4*tx + 1] * inv_S;
        out[4*tx + 2] = so[4*tx + 2] * inv_S;
        out[4*tx + 3] = so[4*tx + 3] * inv_S;
    }
}
...
// ============================================================================
//
// Adapted from TurboFlash architecture for Spiral PQ2 K decode.
//
// Pass 1: Each threadgroup processes a block of BLOCK_SIZE KV positions.
//   - PQ codebook decode → R_inv rotation → mean add → RoPE → Q·K score
//   - Online softmax + F16 V accumulation
//   - Writes per-block partial results: {partial_out[DV], max_score, exp_sum}
//
// Pass 2: One threadgroup per query head merges block partial results.
//   - Online softmax correction across blocks → final output
//   - No inverse WHT needed (Spiral stores V as F16, not WHT-rotated)
//
// Key difference from TurboFlash: K decode requires R_inv 128×128 matmul
// per position, using threadgroup memory for the rotated vector.
//

#define SPIRAL_FLASH_BLOCK_SIZE 256
#define SPIRAL_FLASH_TG_SIZE    128

// ---------------------------------------------------------------------------
// SpiralFlash Pass 1: Block scoring with PQ K decode
// ---------------------------------------------------------------------------
//
// Grid: (n_bh, n_blocks, 1)
//   n_bh = ne01 * ne02 * ne03
//   n_blocks = ceil(n_kv / SPIRAL_FLASH_BLOCK_SIZE)
//
// Threadgroup: (128, 1, 1) — 4 SIMD groups
// Shared memory: s_rot[128] + s_k[128] + s_reduce[4] = 260 floats = 1040 bytes
//
// 128 threads: 1 thread per output dimension for R_inv rotation (4x faster than 32)
// PQ decode uses threads 0-31 (one per subspace)
// Q·K dot uses simd_sum + cross-SIMD reduction via shared memory
//
kernel void kernel_spiral_pq2_flash_p1(
        constant ggml_metal_kargs_spiral_flash_p1 & args [[buffer(0)]],
        device const char  * q           [[buffer(1)]],
        device const char  * k_pq        [[buffer(2)]],
        device const char  * v_f16       [[buffer(3)]],
        device const char  * mask        [[buffer(4)]],
        device       float * partial_out [[buffer(5)]],
        device       float * partial_ms  [[buffer(6)]],
        device const float * codebooks   [[buffer(7)]],
        device const float * R_kv_inv    [[buffer(8)]],
        device const float * means       [[buffer(9)]],
    ...
    const int t_start = (int)(block_id * SPIRAL_FLASH_BLOCK_SIZE);
    const int t_end   = min(t_start + SPIRAL_FLASH_BLOCK_SIZE, T_kv);
    ...
    threadgroup float * s_rot    = shmem;
    threadgroup float * s_k      = shmem + D;
    threadgroup float * s_reduce = shmem + 2 * D;

    device const float * q_ptr = (device const float *)((device const char *)q +
                                  iq1*args.nb01 + iq2*args.nb02 + iq3*args.nb03);
    const float q_val = q_ptr[tid];

    device const float * head_mean = means + ikv2 * D;
    const float mean_val = head_mean[tid];
    ...
    device const half * mask_ptr = (device const half *)(mask +
                                    iq1*args.nb31 +
                                    (iq2 % args.ne32)*args.nb32 +
                                    (iq3 % args.ne33)*args.nb33);
    ...
    const int rope_freq_idx = rope_first_half ? (int)tid : ((int)tid - D/2);
    const float rope_freq = 1.0f / pow(rope_freq_base, (float)(2 * rope_freq_idx) / (float)D);
    ...
    for (int t = t_start; t < t_end; t++) {
        ...
        float mask_val = (float)mask_ptr[t];
        if (mask_val <= -MAXHALF) {
            continue;
        }

        // Step 1: PQ decode → s_rot (threads 0-31 only)
        if (tid < NBLK) {
            device const block_spiral_pq2 * blk =
                (device const block_spiral_pq2 *)(kp + t * args.nb11);
            const float norm = blk->d;
            const uint8_t code = blk->codes[tid];
            device const float * cb_vec = head_cb + tid * NCW * PBS + code * PBS;
            s_rot[tid * PBS + 0] = cb_vec[0] * norm;
            s_rot[tid * PBS + 1] = cb_vec[1] * norm;
            s_rot[tid * PBS + 2] = cb_vec[2] * norm;
            s_rot[tid * PBS + 3] = cb_vec[3] * norm;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Step 2: Direct read from PQ decoded vector (natural space — no R_inv needed)
        {
            s_k[tid] = s_rot[tid] + mean_val;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);

        // Step 3: RoPE via shared memory (freq precomputed before loop)
        float k_roped;
        {
            const int pos = positions[t];
            const float partner_val = rope_first_half ? s_k[tid + 64] : s_k[tid - 64];
            const float theta = (float)pos * rope_freq;
            const float cos_t = cos(theta);
            const float sin_t = sin(theta);
            if (rope_first_half) {
                k_roped = s_k[tid] * cos_t - partner_val * sin_t;
            } else {
                k_roped = partner_val * sin_t + s_k[tid] * cos_t;
            }
        }

        // Step 4: Q·K dot (cross-SIMD reduction)
        float qk_partial = q_val * k_roped;
        float qk_simd = simd_sum(qk_partial);
        if (lane == 0) {
            s_reduce[sg_id] = qk_simd;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        float score;
        if (tid == 0) {
            score = s_reduce[0] + s_reduce[1] + s_reduce[2] + s_reduce[3];
            score = score * args.scale + mask_val;
            s_reduce[0] = score;
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        score = s_reduce[0];

        // Step 5: Online softmax + V accumulation
        {
            float new_m = max(m_state, score);
            float exp_diff  = exp(m_state - new_m);
            float exp_score = exp(score - new_m);
            o_state *= exp_diff;
            device const half * v_row = (device const half *)(vp + t * args.nb21);
            o_state += exp_score * (float)v_row[tid];
    ...
    partial_out[bh_idx * (uint64_t)n_blocks * D + block_id * D + tid] = o_state;
    if (tid == 0) {
...
// ---------------------------------------------------------------------------
// SpiralFlash Pass 2: Merge block partial results (no WHT)
// ---------------------------------------------------------------------------
//
// Grid: (n_bh, 1, 1)
// Threadgroup: (128, 1, 1)
// Shared memory: shared_out[128] + global_max + global_sum = 130 floats
//
kernel void kernel_spiral_pq2_flash_p2(
        constant ggml_metal_kargs_turbo_flash_p2 & args [[buffer(0)]],
        device const float * partial_out [[buffer(1)]],
        device const float * partial_ms  [[buffer(2)]],
    ...
    threadgroup float * shared_global_max = shmem + D;
    threadgroup float * shared_global_sum = shmem + D + 1;

    // ====== Step 1: Find global max and sum across all blocks ======
    if (tid == 0) {
        float gmax = -INFINITY;
    ...
    if ((int)tid < D) {
        ...
        for (int b = 0; b < n_blocks; b++) {
            ...
            float block_val = partial_out[bh_idx * (uint64_t)n_blocks * D + b * D + tid];
    ...
    // ====== Step 3: Write output — no WHT needed for Spiral ======
    // Output layout: [D, n_heads, n_queries, batch]
    // bh_idx encodes (iq1, iq2, iq3) — for decode iq1=0 always
    // So output offset = bh_idx * D (since n_queries=1 for decode)
    if ((int)tid < D) {
        dst[bh_idx * D + tid] = shared_out[tid];
    }
}
...
template [[host_name("kernel_cpy_spiral_3bit_f32")]] kernel cpy_q_f_t kernel_cpy_q_f32<float4x4, block_spiral_3bit, 8, dequantize_spiral_3bit>;
template [[host_name("kernel_cpy_spiral_3bit_f16")]] kernel cpy_q_f_t kernel_cpy_q_f32<half4x4, block_spiral_3bit, 8, dequantize_spiral_3bit>;

// Spiral PQ2 dequant-copy (for ggml_cast from SPIRAL_PQ2 → F32/F16)
template [[host_name("kernel_cpy_spiral_pq2_f32")]] kernel cpy_q_f_t kernel_cpy_q_f32<float4x4, block_spiral_pq2, 8, dequantize_spiral_pq2>;
template [[host_name("kernel_cpy_spiral_pq2_f16")]] kernel cpy_q_f_t kernel_cpy_q_f32<half4x4, block_spiral_pq2, 8, dequantize_spiral_pq2>;

// =====================================================================
// Spiral PQ2 codebook dequant kernel — FULL PQ decode, PARALLELIZED
//
// 128 threads per threadgroup. Each thread handles one output dimension.
// Uses threadgroup shared memory for the codebook-decoded vector so all
// threads can participate in the 128×128 rotation matmul.
//
// Pipeline per (head, kv_position):
//   1. Codebook lookup + norm scale → shared[128] (rotated space)
//   2. Inverse rotation: out[tid] = sum_j shared[j] * R_inv[j*128+tid]
//   3. Add mean carrier: out[tid] += mean[tid]
//   4. Write F32 output (natural pre-RoPE space)
//
// Buffer layout:
//   buffer(0) = args (ggml_metal_kargs_cpy)
//   buffer(1) = src0 (SPIRAL_PQ2 blocks)
//   buffer(2) = dst  (F32 output)
//   buffer(3) = codebooks  [n_kv_heads * 32 * 256 * 4] F32
//   buffer(4) = R_kv_inv   [128 * 128] F32
//   buffer(5) = means      [n_kv_heads * 128] F32
//
// Dispatch: (ne01, ne02, ne03) threadgroups × 128 threads each
// =====================================================================
kernel void kernel_cpy_spiral_pq2_cb_f32(
        constant ggml_metal_kargs_cpy & args,
        device  const char  * src0,
        device        char  * dst,
        device  const float * codebooks,
        device  const float * R_kv_inv,
        device  const float * means,
        uint3   tgpig[[threadgroup_position_in_grid]],
        ushort  tiitg[[thread_index_in_threadgroup]],
        ushort3   ntg[[threads_per_threadgroup]]) {

    constexpr int D = 128;       // QK_SPIRAL_PQ2
    constexpr int PQ_BS = 4;
    constexpr int N_BLOCKS = D / PQ_BS;  // 32
    constexpr int N_CW = 256;

    // Thread identity: tiitg = 0..127, one per output dimension
    const int tid = tiitg;

    // Each threadgroup handles one (head, position)
    const int i03 = tgpig[2];
    const int i02 = tgpig[1];
    ...
    // Compute flat element index for destination mapping
    const int64_t n = i03*args.ne02*args.ne01*args.ne00 + i02*args.ne01*args.ne00 + i01*args.ne00;
    ...
    device const block_spiral_pq2 * src_blk = (device const block_spiral_pq2 *)(src0 + i03*args.nb03 + i02*args.nb02 + i01*args.nb01);

    // Destination F32 pointer
    device float * dst_row = (device float *)(dst + i3*args.nb3 + i2*args.nb2 + i1*args.nb1 + i0*args.nb0);

    // Head index for codebook and mean lookup
    const int head_idx = i01;
    const device float * head_cb   = codebooks + head_idx * N_BLOCKS * N_CW * PQ_BS;
    const device float * head_mean = means + head_idx * D;

    const float norm = src_blk->d;

    // Shared memory: decoded rotated vector (128 floats)
    threadgroup float s_rot[D];

    // Step 1: Codebook lookup + norm scale → shared memory
    // First 32 threads each load one subspace (4 values)
    if (tid < N_BLOCKS) {
        const uint8_t code = src_blk->codes[tid];
        const device float * cb_vec = head_cb + tid * N_CW * PQ_BS + code * PQ_BS;
        s_rot[tid * PQ_BS + 0] = cb_vec[0] * norm;
        s_rot[tid * PQ_BS + 1] = cb_vec[1] * norm;
        s_rot[tid * PQ_BS + 2] = cb_vec[2] * norm;
        s_rot[tid * PQ_BS + 3] = cb_vec[3] * norm;
    }
    threadgroup_barrier(mem_flags::mem_threadgroup);

    // Step 2: Direct read from PQ decoded vector (natural space — no R_inv needed)
    float result = s_rot[tid] + head_mean[tid];
...
// =====================================================================
// Spiral PQ2 SET_ROWS kernel — PQ codebook encoding with scalar fallback
//
// Buffer layout:
//   buffer(0) = args (kargs struct)
//   buffer(1) = src0 (F32 input data)
//   buffer(2) = src1 (indices)
//   buffer(3) = dst  (KV cache — block_spiral_pq2)
//   buffer(4) = codebooks (optional — [n_kv_heads * 32 * 256 * 4] float32)
//   buffer(5) = means     (optional — [n_kv_heads * 128] float32)
//   buffer(6) = R_kv      (optional — [128 * 128] float32)
//
// When buffer(4) is present: PQ codebook encoding
// When buffer(4) is absent:  scalar 2-bit Lloyd-Max fallback
// =====================================================================
template <typename TI>
kernel void kernel_set_rows_spiral_pq2(
        constant ggml_metal_kargs_set_rows & args,
        device const  void * src0,
        device const  void * src1,
        device       float * dst,
    ...
    const TI      i1  = ((const device TI *) ((const device char *) src1 + i10*args.nb10 + i11*args.nb11 + i12*args.nb12))[0];
          ...
          device block_spiral_pq2 * dst_row = (      device block_spiral_pq2 *) ((      device char *) dst +  i1*args.nb1  + i02*args.nb2  + i03*args.nb3);
    const device float             * src_row = (const device float            *) ((const device char *) src0 + i01*args.nb01 + i02*args.nb02 + i03*args.nb03);

    const int n_blocks = args.nk0;

    for (int blk = tiitg%tptg.x; blk < n_blocks; blk += tptg.x) {
        const device float * blk_src = src_row + QK_SPIRAL_PQ2 * blk;

        // Scalar 2-bit Lloyd-Max encoding (fallback — used when no codebooks)
        float norm_sq = 0.0f;
        ...
        const float spiral_centroids_2bit[4] = { -0.133462f, -0.039994f, 0.039994f, 0.133462f };
        const float spiral_mid_2bit[3] = { -0.086728f, 0.0f, 0.086728f };

        float recon_norm_sq = 0.0f;

        for (int j = 0; j < 32; j++) {
            dst_row[blk].codes[j] = 0;
        }

        for (int j = 0; j < QK_SPIRAL_PQ2; j++) {
            float val = blk_src[j] * inv_norm;
            uint8_t idx;
            if      (val < spiral_mid_2bit[0]) idx = 0;
            else if (val < spiral_mid_2bit[1]) idx = 1;
            else if (val < spiral_mid_2bit[2]) idx = 2;
            else                               idx = 3;

            dst_row[blk].codes[j / 4] |= (idx & 0x3) << ((j % 4) * 2);

            float c = spiral_centroids_2bit[idx];
            recon_norm_sq += c * c;
        }

        float recon_norm = sqrt(recon_norm_sq);
        dst_row[blk].d = (recon_norm > 1e-10f) ? norm / recon_norm : norm;
    }
}

// PQ codebook version of set_rows — separate kernel with extra buffer args
template <typename TI>
kernel void kernel_set_rows_spiral_pq2_cb(
        constant ggml_metal_kargs_set_rows & args,
        device const  void  * src0,
        device const  void  * src1,
        device       float  * dst,
        device const float  * codebooks,  // [n_kv_heads * 32 * 256 * 4]
        device const float  * means,      // [n_kv_heads * 128]
        device const float  * R_kv,       // [128 * 128]
        uint3                tgpig[[threadgroup_position_in_grid]],
        uint                 tiitg[[thread_index_in_threadgroup]],
        uint3                tptg [[threads_per_threadgroup]]) {

    constexpr int D = QK_SPIRAL_PQ2;    // 128
    constexpr int PQ_BS = 4;
    constexpr int N_BLOCKS = D / PQ_BS; // 32
    constexpr int N_CW = 256;
    ...
    for (int blk = tiitg%tptg.x; blk < n_blocks; blk += tptg.x) {
        const device float * blk_src = src_row + D * blk;
        const int head_idx = blk;
        ...
        float centered[D];
        for (int j = 0; j < D; j++) {
            centered[j] = blk_src[j] - head_mean[j];
        }

        // Step 2: Natural space — no rotation needed, copy centered to rotated
        float rotated[D];
        for (int i = 0; i < D; i++) {
            rotated[i] = centered[i];
        }

        // Step 3: L2 normalize
        float norm_sq = 0.0f;
        for (int j = 0; j < D; j++) {
            norm_sq += rotated[j] * rotated[j];
        }
        float norm = sqrt(norm_sq);
        ...
        float unit_vec[D];
        for (int j = 0; j < D; j++) {
            unit_vec[j] = rotated[j] * inv_norm;
        }

        // Step 4: PQ snap — find nearest codeword per 4D subspace
        float recon_norm_sq = 0.0f;

        for (int b = 0; b < N_BLOCKS; b++) {
            const device float * cb_block = head_cb + b * N_CW * PQ_BS;
            float best_dist = 1e30f;
            uint8_t best_code = 0;

            for (int cw = 0; cw < N_CW; cw++) {
                const device float * cb_vec = cb_block + cw * PQ_BS;
                float dist = 0.0f;
                for (int d = 0; d < PQ_BS; d++) {
                    float diff = unit_vec[b * PQ_BS + d] - cb_vec[d];
                    dist += diff * diff;
                }
                if (dist < best_dist) {
                    best_dist = dist;
                    best_code = (uint8_t)cw;
                }
            }

            dst_row[blk].codes[b] = best_code;

            // Accumulate reconstruction norm
            const device float * best_vec = cb_block + best_code * PQ_BS;
            for (int d = 0; d < PQ_BS; d++) {
                recon_norm_sq += best_vec[d] * best_vec[d];
            }
        }
...
template [[host_name("kernel_get_rows_spiral_3bit")]] kernel get_rows_q_t kernel_get_rows_q<block_spiral_3bit, 8, dequantize_spiral_3bit>;
...
// Spiral PQ2 set_rows instantiations
typedef decltype(kernel_set_rows_spiral_pq2<int64_t>) set_rows_spiral_pq2_t;
template [[host_name("kernel_set_rows_spiral_pq2_i64")]] kernel set_rows_spiral_pq2_t kernel_set_rows_spiral_pq2<int64_t>;
template [[host_name("kernel_set_rows_spiral_pq2_i32")]] kernel set_rows_spiral_pq2_t kernel_set_rows_spiral_pq2<int32_t>;

typedef decltype(kernel_set_rows_spiral_pq2_cb<int64_t>) set_rows_spiral_pq2_cb_t;
template [[host_name("kernel_set_rows_spiral_pq2_cb_i64")]] kernel set_rows_spiral_pq2_cb_t kernel_set_rows_spiral_pq2_cb<int64_t>;
template [[host_name("kernel_set_rows_spiral_pq2_cb_i32")]] kernel set_rows_spiral_pq2_cb_t kernel_set_rows_spiral_pq2_cb<int32_t>;
...
template [[host_name("kernel_mul_mm_spiral_3bit_f32")]]         kernel mul_mm_t kernel_mul_mm<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_spiral_3bit,  8,  dequantize_spiral_3bit,          float,  float4x4,  float, float2x4>;
template [[host_name("kernel_mul_mm_spiral_3bit_rotated_f32")]] kernel mul_mm_t kernel_mul_mm<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_spiral_3bit,  8,  dequantize_spiral_3bit_rotated,  float,  float4x4,  float, float2x4>;
...
template [[host_name("kernel_mul_mm_spiral_3bit_f16")]]         kernel mul_mm_t kernel_mul_mm<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_spiral_3bit,  8,  dequantize_spiral_3bit,          float,  float4x4,  half, half2x4>;
template [[host_name("kernel_mul_mm_spiral_3bit_rotated_f16")]] kernel mul_mm_t kernel_mul_mm<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_spiral_3bit,  8,  dequantize_spiral_3bit_rotated,  float,  float4x4,  half, half2x4>;
...
template [[host_name("kernel_mul_mm_id_spiral_3bit_f32")]]     kernel mul_mm_id kernel_mul_mm_id<half,   half4x4,   simdgroup_half8x8,   half,   half2x4,   simdgroup_half8x8,   block_spiral_3bit,  8,  dequantize_spiral_3bit,  float,  float4x4,  float, float2x4>;
...
template [[host_name("kernel_mul_mv_id_spiral_3bit_f32")]]  kernel kernel_mul_mv_id_t kernel_mul_mv_id<mmv_fn<kernel_mul_mv_spiral_3bit_f32_impl<N_R0_SPIRAL_3BIT>>>;
