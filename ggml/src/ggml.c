...
static const struct ggml_type_traits type_traits[GGML_TYPE_COUNT] = {
    ...
    [GGML_TYPE_SPIRAL_PQ2] = {
        .type_name                = "spiral_pq2",
        .blck_size                = QK_SPIRAL_PQ2,
        .type_size                = sizeof(block_spiral_pq2),
        .is_quantized             = true,
        .to_float                 = (ggml_to_float_t) dequantize_row_spiral_pq2,
        .from_float_ref           = (ggml_from_float_t) quantize_row_spiral_pq2_ref,
    },
    [GGML_TYPE_SPIRAL_3BIT] = {
        .type_name                = "spiral_3bit",
        .blck_size                = QK_SPIRAL_3BIT,
        .type_size                = sizeof(block_spiral_3bit),
        .is_quantized             = true,
...
// ggml_spiral_rotate
struct ggml_tensor * ggml_spiral_rotate(
        struct ggml_context * ctx,
        struct ggml_tensor  * a,
        struct ggml_tensor  * signs,
        struct ggml_tensor  * perm,
        int                   direction) {
    GGML_ASSERT(ggml_is_contiguous(a));
    GGML_ASSERT(a->type == GGML_TYPE_F32);
    GGML_ASSERT(signs->type == GGML_TYPE_F32);
    GGML_ASSERT(signs->ne[0] == a->ne[0] || signs->ne[0] * (a->ne[1] > 0 ? a->ne[1] : 1) <= ggml_nelements(a));
    struct ggml_tensor * result = ggml_new_tensor(ctx, GGML_TYPE_F32, 4, a->ne);
    result->op = GGML_OP_SPIRAL_ROTATE;
    result->src[0] = a;
    result->src[1] = signs;
    result->src[2] = perm;
