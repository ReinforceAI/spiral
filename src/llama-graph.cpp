...
void llm_graph_input_attn_kv::set_input(const llama_ubatch * ubatch) {
    ...
    if (spiral_k_pos) {
        mctx->set_input_spiral_k_pos(spiral_k_pos);
    }
}
...
ggml_tensor * llm_graph_context::spiral_rotate_activation(ggml_tensor * cur, int64_t dim) const {
    ...
    // Pass 1: signs1 → FWHT (no perm)
    cur = ggml_spiral_rotate(ctx0, cur, rot->signs1, NULL, 0);
    // Pass 2: perm → signs2 → FWHT
    cur = ggml_spiral_rotate(ctx0, cur, rot->signs2, rot->perm1, 0);

    // Pass 3 (if needed, e.g. D=18944)
    if (rot->n_passes >= 3 && rot->signs3 && rot->perm2) {
        cur = ggml_spiral_rotate(ctx0, cur, rot->signs3, rot->perm2, 0);
    }
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // Spiral pre-RoPE path: dequant cached K → RoPE → F16
    const bool spiral_use_fused_fa = (k->type == GGML_TYPE_SPIRAL_PQ2 &&
                                      cparams.flash_attn && !cparams.auto_fa &&
                                      mctx_cur->get_spiral_k_codebooks(il) != nullptr);

    if (spiral_use_fused_fa) {
        // V: cast to F16
        if (v->type == GGML_TYPE_SPIRAL_PQ2) {
            v = ggml_cast(ctx0, v, GGML_TYPE_F16);
        }
        ...
        ggml_tensor * q_fa = ggml_view_4d(ctx0, q, q->ne[0], q->ne[1], q->ne[2]/n_stream, n_stream,
        ...
        ggml_flash_attn_ext_set_prec(cur_fa, GGML_PREC_F32);

        // Attach codebooks for fused PQ kernel dispatch
        cur_fa->src[5] = mctx_cur->get_spiral_k_codebooks(il);
        cur_fa->src[6] = mctx_cur->get_spiral_cb_rotation_inv();
        cur_fa->src[7] = mctx_cur->get_spiral_k_means(il);
        cur_fa->src[8] = inp->spiral_k_pos;

        ggml_tensor * cur = ggml_reshape_2d(ctx0, cur_fa, cur_fa->ne[0]*cur_fa->ne[1], cur_fa->ne[2]*cur_fa->ne[3]);
        cb(cur, "kqv_out", il);
    ...
    if (k->type == GGML_TYPE_SPIRAL_PQ2) {
        k = ggml_cast(ctx0, k, GGML_TYPE_F32);

        ggml_tensor * k_cb   = mctx_cur->get_spiral_k_codebooks(il);
        ggml_tensor * r_inv  = mctx_cur->get_spiral_cb_rotation_inv();
        ggml_tensor * k_mean = mctx_cur->get_spiral_k_means(il);
        if (k_cb && r_inv && k_mean) {
            k->src[2] = k_cb;
            k->src[3] = r_inv;
            k->src[4] = k_mean;
        }
