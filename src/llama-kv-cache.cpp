...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ggml_init_params params = {
                // +5 for rotation matrices (turbo: 3, spiral: 2)
                /*.mem_size   =*/ size_t((2u*(1 + n_stream)*n_layer_kv + 5)*ggml_tensor_overhead()),
                /*.mem_buffer =*/ NULL,
                /*.no_alloc   =*/ true,
            };
    ...
    for (uint32_t il = 0; il < hparams.n_layer; il++) {
        ...
        // Spiral: create R_kv rotation matrix tensors (once, shared across layers)
        if (spiral_rotation == nullptr && type_k == GGML_TYPE_SPIRAL_PQ2) {
            spiral_rotation = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 128, 128);
            ggml_format_name(spiral_rotation, "spiral_rotation");
            spiral_rotation_inv = ggml_new_tensor_2d(ctx, GGML_TYPE_F32, 128, 128);
            ggml_format_name(spiral_rotation_inv, "spiral_rotation_inv");
        }
    }
    ...
    for (auto & [buft, ctx] : ctx_map) {
        ...
        // Fill spiral rotation matrices AFTER buffer clear
        if (spiral_rotation != nullptr && spiral_rotation->buffer != nullptr && !model.hparams.no_alloc) {
            #include "spiral-rotation-data.h"
            ggml_backend_tensor_set(spiral_rotation, SPIRAL_ROTATION_R, 0, 128 * 128 * sizeof(float));
            ggml_backend_tensor_set(spiral_rotation_inv, SPIRAL_ROTATION_RT, 0, 128 * 128 * sizeof(float));
            LLAMA_LOG_INFO("%s: Spiral R_kv rotation matrices initialized (128x128)\n", __func__);
        }
    ...
    // Auto-load Spiral codebooks for PQ KV cache and/or SPIRAL_3BIT weight rotation
    // Priority: 1) SPIRAL_CODEBOOK_PATH env var (set by spiral-chat/spiral-serve wrappers)
    //           2) codebooks.spiralcb in current working directory (manual usage)
    const char * cb_env = getenv("SPIRAL_CODEBOOK_PATH");
    const bool need_codebooks = (type_k == GGML_TYPE_SPIRAL_PQ2 || type_v == GGML_TYPE_SPIRAL_PQ2 ||
                                  (cb_env && cb_env[0] != '\0'));

    if (need_codebooks) {
        std::string cb_path;

        if (cb_env && cb_env[0] != '\0') {
            cb_path = cb_env;
        } else {
            // Auto-discover: try codebooks.spiralcb in current working directory
            FILE * f = fopen("codebooks.spiralcb", "rb");
            if (f) {
                fclose(f);
                cb_path = "codebooks.spiralcb";
                LLAMA_LOG_INFO("%s: auto-discovered spiral codebooks: %s\n", __func__, cb_path.c_str());
            }
        }

        if (!cb_path.empty()) {
            if (!load_spiral_codebooks(cb_path)) {
                LLAMA_LOG_WARN("%s: failed to load spiral codebooks from %s\n", __func__, cb_path.c_str());
                if (type_k == GGML_TYPE_SPIRAL_PQ2) {
                    LLAMA_LOG_WARN("%s: PQ KV cache will use scalar fallback (lower quality)\n", __func__);
                }
            }
        } else {
            if (type_k == GGML_TYPE_SPIRAL_PQ2) {
                LLAMA_LOG_WARN("%s: no spiral codebooks found — PQ KV will use scalar fallback\n", __func__);
                LLAMA_LOG_WARN("%s: place codebooks.spiralcb in current directory or set SPIRAL_CODEBOOK_PATH\n", __func__);
            }
        }
    }
}

bool llama_kv_cache::load_spiral_codebooks(const std::string & codebook_path) {
    if (spiral_codebooks != nullptr) {
        LLAMA_LOG_WARN("%s: codebooks already loaded, skipping\n", __func__);
        return true;
    }

    // Determine buffer type — use same as KV cache layer 0
    if (layers.empty()) {
        LLAMA_LOG_ERROR("%s: no KV cache layers, cannot load codebooks\n", __func__);
        return false;
    }

    ggml_tensor * k0 = layers[0].k;
    if (!k0 || !k0->buffer) {
        LLAMA_LOG_ERROR("%s: KV cache not allocated, cannot determine buffer type\n", __func__);
        return false;
    }
    ggml_backend_buffer_type_t buft = ggml_backend_buffer_get_type(k0->buffer);

    // Create a separate ggml context for codebook tensors
    // We need: 2 rotation tensors + n_layers * 3 tensors (k_cb, v_cb, k_means)
    const uint32_t n_layer = hparams.n_layer;
    const size_t n_tensors = 2 + n_layer * 3 + 16; // generous margin
    ggml_init_params params = {
    ...
    ggml_context * ctx = ggml_init(params);
    if (!ctx) {
        LLAMA_LOG_ERROR("%s: failed to create ggml context for codebooks\n", __func__);
        return false;
    }

    spiral_codebooks = new spiral_codebook_data();
    spiral_codebooks->ctx = ctx;  // transfer ownership

    bool ok = spiral_codebook_load(codebook_path, *spiral_codebooks, ctx, buft);
    if (!ok) {
        delete spiral_codebooks;  // destructor frees ctx and buf
        spiral_codebooks = nullptr;
        return false;
    }

    // NOTE: rotation copy from codebook to KV cache tensors is skipped for now.
    // Rotation is disabled for scalar 2-bit. When PQ codebooks are used with rotation,
    // the rotation matrices will be read directly from spiral_codebooks->R_kv.

    LLAMA_LOG_INFO("%s: spiral PQ codebooks loaded successfully\n", __func__);
    return true;
}

ggml_tensor * llama_kv_cache::get_spiral_k_codebooks(int32_t il) const {
    if (!spiral_codebooks || !spiral_codebooks->loaded) return nullptr;
    const int32_t ikv = map_layer_ids.at(il);
    if (ikv < 0 || (uint32_t)ikv >= spiral_codebooks->layers.size()) return nullptr;
    return spiral_codebooks->layers[ikv].k_codebooks;
}

ggml_tensor * llama_kv_cache::get_spiral_v_codebooks(int32_t il) const {
    if (!spiral_codebooks || !spiral_codebooks->loaded) return nullptr;
    const int32_t ikv = map_layer_ids.at(il);
    if (ikv < 0 || (uint32_t)ikv >= spiral_codebooks->layers.size()) return nullptr;
    return spiral_codebooks->layers[ikv].v_codebooks;
}

ggml_tensor * llama_kv_cache::get_spiral_k_means(int32_t il) const {
    if (!spiral_codebooks || !spiral_codebooks->loaded) return nullptr;
    const int32_t ikv = map_layer_ids.at(il);
    if (ikv < 0 || (uint32_t)ikv >= spiral_codebooks->layers.size()) return nullptr;
    return spiral_codebooks->layers[ikv].k_means;
}

ggml_tensor * llama_kv_cache::get_spiral_cb_rotation_inv() const {
    if (!spiral_codebooks || !spiral_codebooks->loaded) return nullptr;
    return spiral_codebooks->R_kv_inv;
}

const spiral_weight_rotation * llama_kv_cache::get_spiral_weight_rotation(int64_t dim) const {
    if (!spiral_codebooks || !spiral_codebooks->loaded) return nullptr;
    return spiral_codebooks->get_weight_rotation(dim);
}
...
void llama_kv_cache::clear(bool data) {
    ...
    if (data) {
        ...
        // Re-initialize spiral rotation matrices after buffer clear
        if (spiral_rotation != nullptr && spiral_rotation->buffer != nullptr) {
            #include "spiral-rotation-data.h"
            ggml_backend_tensor_set(spiral_rotation, SPIRAL_ROTATION_R, 0, 128 * 128 * sizeof(float));
            ggml_backend_tensor_set(spiral_rotation_inv, SPIRAL_ROTATION_RT, 0, 128 * 128 * sizeof(float));
        }
...
ggml_tensor * llama_kv_cache::cpy_k(ggml_context * ctx, ggml_tensor * k_cur, ggml_tensor * k_idxs, int32_t il, const slot_info & sinfo) const {
    ...
    // Spiral pre-RoPE path: when PQ codebooks are loaded, the set_rows kernel
    // handles mean subtraction, R_kv rotation, and PQ codebook search internally.
    // When no codebooks: scalar 2-bit Lloyd-Max (no rotation, no mean subtraction).

    // store the current K values into the cache
    ggml_tensor * result = ggml_set_rows(ctx, k, k_cur, k_idxs);

    // Attach PQ codebook data for the Metal kernel
    if (k->type == GGML_TYPE_SPIRAL_PQ2 && spiral_codebooks != nullptr && spiral_codebooks->loaded) {
        const int32_t ikv = map_layer_ids.at(il);
        ggml_tensor * k_cb   = spiral_codebooks->layers[ikv].k_codebooks;
        ggml_tensor * k_mean = spiral_codebooks->layers[ikv].k_means;
        ggml_tensor * r_kv   = spiral_codebooks->R_kv;

        if (k_cb && k_mean && r_kv) {
            result->src[3] = k_cb;    // codebooks [n_kv_heads * 32 * 256 * 4] F32
            result->src[4] = k_mean;  // means [n_kv_heads * 128] F32
            result->src[5] = r_kv;    // R_kv [128 * 128] F32
        }
    }
...
ggml_tensor * llama_kv_cache::cpy_v(ggml_context * ctx, ggml_tensor * v_cur, ggml_tensor * v_idxs, int32_t il, const slot_info & sinfo) const {
    ...
    if (!v_trans) {
        ...
        // V PQ encoding disabled for now — V uses scalar path
        // TODO: Add zero-filled means tensor for V PQ encode
...
void llama_kv_cache::set_input_spiral_k_pos(ggml_tensor * dst, uint32_t n_kv, const slot_info & sinfo) const {
    GGML_ASSERT(ggml_backend_buffer_is_host(dst->buffer));
...
ggml_tensor * llama_kv_cache_context::get_spiral_rotation() const {
    return kv->get_spiral_rotation();
}
ggml_tensor * llama_kv_cache_context::get_spiral_rotation_inv() const {
    return kv->get_spiral_rotation_inv();
}
ggml_tensor * llama_kv_cache_context::get_spiral_k_codebooks(int32_t il) const {
    return kv->get_spiral_k_codebooks(il);
}
ggml_tensor * llama_kv_cache_context::get_spiral_v_codebooks(int32_t il) const {
    return kv->get_spiral_v_codebooks(il);
}
ggml_tensor * llama_kv_cache_context::get_spiral_k_means(int32_t il) const {
    return kv->get_spiral_k_means(il);
}
ggml_tensor * llama_kv_cache_context::get_spiral_cb_rotation_inv() const {
    return kv->get_spiral_cb_rotation_inv();
}
const spiral_weight_rotation * llama_kv_cache_context::get_spiral_weight_rotation(int64_t dim) const {
    return kv->get_spiral_weight_rotation(dim);
}
...
void llama_kv_cache_context::set_input_spiral_k_pos(ggml_tensor * dst) const {
    kv->set_input_spiral_k_pos(dst, n_kv, sinfos[i_cur]);
}
