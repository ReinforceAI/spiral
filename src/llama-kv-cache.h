...
public:
    ...
    // Spiral: get R_kv rotation matrices
    ggml_tensor * get_spiral_rotation() const { return spiral_rotation; }
    ggml_tensor * get_spiral_rotation_inv() const { return spiral_rotation_inv; }

    // Spiral PQ codebook accessors (returns nullptr if codebooks not loaded)
    ggml_tensor * get_spiral_k_codebooks(int32_t il) const;
    ggml_tensor * get_spiral_v_codebooks(int32_t il) const;
    ggml_tensor * get_spiral_k_means(int32_t il) const;
    ...
    // Load PQ codebooks from a .spiralcb file
    bool load_spiral_codebooks(const std::string & codebook_path);
    ...
    // Spiral pre-RoPE path: fill cached K positions for post-dequant RoPE application
    void set_input_spiral_k_pos(ggml_tensor * dst, uint32_t n_kv, const slot_info & sinfo) const;
...
private:
    ...
    // Spiral R_kv rotation matrices (128x128, random orthogonal from seed 0x524F5432)
    ggml_tensor * spiral_rotation = nullptr;      // R_kv (forward)
    ggml_tensor * spiral_rotation_inv = nullptr;   // R_kv^T (inverse)

    // Spiral PQ codebook data — loaded from .spiralcb file
    // Contains per-layer codebooks, mean carriers, and rotation matrices.
    // Null until load_spiral_codebooks() is called.
    struct spiral_codebook_data * spiral_codebooks = nullptr;
...
public:
    ...
    // Spiral rotation accessors
    ggml_tensor * get_spiral_rotation() const;
    ggml_tensor * get_spiral_rotation_inv() const;
    ...
    void set_input_k_shift   (ggml_tensor * dst) const;
    void set_input_spiral_k_pos(ggml_tensor * dst) const;
    void set_input_kq_mask   (ggml_tensor * dst, const llama_ubatch * ubatch, bool causal_attn) const;
    void set_input_pos_bucket(ggml_tensor * dst, const llama_ubatch * ubatch) const;
