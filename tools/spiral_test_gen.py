...
def write_gguf_spiral_block_format(row_norms: np.ndarray, packed_codes: np.ndarray) -> bytes:
    ...
    EACH block has the row_norm in its first 2 bytes (kernel reads xb->norm
    per-block, so every block needs the same row_norm value).
    ...
    norm_arr = np.frombuffer(norm_bytes, dtype=np.uint8).reshape(m, 2)  # [m, 2]
    # Broadcast across all blocks
    out[:, :, :2] = norm_arr[:, None, :]  # [m, nb, 2]
